...
/** 대체 확인 다이얼로그 — 탈출구 진입 버튼. 갈아타기 전에 *이 기기 세션*의
 *  세이브 코드를 받아 둘 수 있는 경로 (돌아올 길을 만들고 나서 대체). */
export const REPLACE_RESCUE_CODE = "현재 진행의 세이브 코드 받기";

/** 대체 확인 다이얼로그 — 탈출구 실패 공용 문구. 세션 없음(401) / 밴 세션 /
 *  서버 오류 / 네트워크 실패 — 원인은 달라도 사용자에게 남는 사실은 하나다:
 *  코드를 못 받았고, 그래도 갈아타기는 계속 진행할 수 있다.
 *  정직한 인프라 언어 + 조건부 어법 (이 기기에 진행이 없을 수도 있으므로). */
export const REPLACE_RESCUE_FAILED =
  "이 기기의 세이브 코드를 받지 못했습니다. 이 기기에 남아 있는 진행이 있다면, 지금 계속할 경우 그 진행으로 돌아올 방법이 없습니다.";
...
/** 채팅 — 발급된 코드 안내. 발급은 여전히 idempotent 지만 회전이 생긴 이상
 *  "바뀌지 않는다"고 약속하면 거짓 — 같은 코드가 유지되는 *조건*을 밝힌다. */
export const SAVE_CODE_ISSUED_NOTE =
  "이 코드를 적어 두면 다른 기기에서 이어할 수 있습니다. 새 코드로 바꾸기 전까지는 다시 확인해도 같은 코드가 나옵니다.";

/** 채팅 — 코드 패널에서 새 코드로 갈아타는(회전) 진입 버튼.
 *  확인 단계의 실행 버튼도 같은 라벨 — 누른 것과 확정하는 것이 같은 행동이다. */
...
/** 채팅 — 세이브 코드 넛지 트리거 임계값 (이 세션의 누적 플레이어 턴 수).
 *  튜닝 값의 단일 홈: rules YAML 은 서버 것이고 이 값을 클라이언트로 나르는
 *  전송 경로가 없으므로, 프론트 튜닝 값은 문구와 같은 집에 산다.
 *  판정 자체는 saveCodeNudge.ts 의 순수 함수 — 이 숫자를 코드에 인라인 금지. */
export const SAVE_CODE_NUDGE_AFTER_TURNS = 6;

/** 채팅 — 세이브 코드 넛지 본문. 시스템 톤(정직한 인프라 언어): 쿠키가 사라지면
 *  진행을 잃을 수 있다는 사실과, 코드를 받아 두면 이어할 수 있다는 사실만 말한다.
 *  NPC 디제틱 말투 금지 — 이건 세계 안 목소리가 아니라 시스템 안내다. */
export const SAVE_CODE_NUDGE_BODY =
  "이 브라우저의 접속 정보가 사라지면 지금까지의 대화를 잃을 수 있습니다. 세이브 코드를 받아 두면 다른 기기에서도 이어할 수 있습니다.";

/** 채팅 — 넛지의 행동 버튼. 헤더의 발급 버튼과 같은 경로를 열지만, 넛지가
 *  말한 일("받아 두면 이어할 수 있다")을 그 자리에서 할 수 있어야 한다 —
 *  안내만 하고 방법은 찾게 만들면 넛지가 아니라 잔소리다. 라벨이 헤더 버튼과
 *  달라야 두 버튼이 화면에 같이 있어도 무엇을 누르는지 헷갈리지 않는다. */
export const SAVE_CODE_NUDGE_ACTION = "세이브 코드 받기";

/** 채팅 — 넛지 닫기 버튼. 한 번 닫으면 이 기기에서 반복 노출되지 않는다. */
export const SAVE_CODE_NUDGE_DISMISS = "나중에 하기";
