...
도착한 자들은 (대체로) *과거의 플레이어*다. 섬을 떠나지 못한 채 망각에 잠긴 자가 NPC가 되어, 다음에 흘러온 자를 맞는다. 4 NPC 모두 이 계열 — 한때 이름과 자아를 가졌으나 떠나지 못해 트로프 직함만 남은 자들. **루프는 닫혀 있다:** 현재의 플레이어도 가장 최근 도착자이며, 깨어나 떠나지 못하면 같은 운명을 따른다. (이 전제는 *authoring 캐논*이다 — 게임은 이를 binary로 노출하지 않는다. "꿈인지 현실인지 모호"가 유지된다. ADR 0025.)

## 플레이어

풍랑을 만나 죽기 직전 *반포기 상태*에서 흘러옴. 자기도 망각의 대상이었으나, NPC와 대화하며 자기 자신의 깨어남도 동시 진행. 플레이어는 NPC들과 *같은 계열의 가장 최근 도착자* — 떠나면 깨어난 자, 머물면 다음 NPC.
...
이 비대칭은 *루프의 핵심 힌트*다. 도착자가 모두 망각으로 이름을 잃는데 혜안만 이름을 붙들었다는 사실이, "이들은 한때 이름 가진 자아였다 = 과거의 플레이어"라는 구조를 드러내는 단서가 된다 (ADR 0025). 단, 힌트 수준 — 디제틱 binary reveal은 하지 않는다.
...
망각 *부분 실패*. 섬에서 *가장 오래 머무른 도착자* — 그래서 가장 많은 도착과 이탈을 지켜봤고, 루프를 인지하기 시작한다. *time*-loop awareness.

ecology 자리: "시간이 사람을 데려가는 것을 본 자". 오랜 세월 다른 도착자(과거 플레이어)들이 떠나거나 사라지는 것을 지켜본 자 — 상실의 대상은 떠나간 자들이자 단절된 외부 세계다 (core_wound `loss`). 시그니처: "나… 이 대화 수백 번 했어." (ADR 0025: 가장 오래 머문 자 = 루프 관찰자.)
