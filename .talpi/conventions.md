...
- step 2: `docs/retros/2026-08-19-talpi-0.5.0-dogfood.md` 편입(자체 커밋).
- step 3: `docs/mechanic-spec.md` — ESC 메뉴를 v1 목표로 유지(사람 결정)
  하되 "현재 앱에 ESC 메뉴가 없고 배포된 진입점은 타이틀 버튼 + 채팅
  헤더 패널" 이라는 잠정 노트를 붙였다. ESC 항목 목록에 회전 추가
  (`발급/입력` → `발급/입력/새 코드로 바꾸기`), 근거는 ADR 0038 위임.
  타이틀 439줄은 설계 의도를 그대로 두고 잠정 노트를 가리키게만 했다
  (같은 사실을 한 문서 안에서 두 번 쓰지 않기 위해).
  **오케스트레이터 교정 1건**: 초안이 "ESC 가 서면 이 진입점들이 그
  안으로 들어온다" 고 썼는데, ESC 는 진행 중인 세션을 전제하므로
  타이틀 진입점은 들어갈 수 없다 — 다음 빌더에게 거짓 지시가 된다.
  '대화 중 진입점만 흡수, 타이틀 진입점은 타이틀에 남고 최종 형태는
  미정' 으로 좁혔다.
  **미조치(같은 종류의 drift, 사람 결정 필요)**: 타이틀의 엔딩 저널
  리스트와 `### 엔딩 저널` 블록도 미구현 — ESC 와 같은 판정(여전히 v1
  목표인가)이 필요하나 이번 판정 범위 밖.
  검증 [FIX] 1건 + 후속 1건 (같은 문단, 둘 다 '다음 빌더에게 거짓 지시'
  방지):
  (a) 진입점을 두 곳이라 썼는데 실제로는 세 곳 — 넛지 배너의
      [세이브 코드 받기] 도 같은 패널을 연다. 문장이 *무엇을* 세는지
      (플레이어가 스스로 여는 표면, 코드를 얻는 모든 경로가 아니라)
      를 앞에 명시해 같은 오독을 막았다. 다이얼로그 탈출구는 독립
      진입점이 아니라 redeem 흐름 안의 한 단계라 세지 않는다(그 사실의
      집은 이미 mechanic-spec 110줄).
