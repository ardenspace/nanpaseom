...
- [x] `App.tsx` applyTurn+sendTurn 훅 추출(동작 보존) + `/turn` 401 자동
      복구 경로 회귀 pin — B5 재노출 조건이 이 경로와 맞물리므로 여기서
      고정한다.
- [x] 회전 UI — 세이브 코드 보기 경로에서 도달, "이전 코드는 더 이상 쓸
...
- [x] 계약 pin: B4 — 무조건 확인(진행 유무로 건너뛰지 않음) 회귀, 상시
      입구 회귀, 탈출구 존재와 실패 분기(401/밴/서버 오류/네트워크에서
      솔직한 문구 + redeem 계속 가능)를 실패 테스트로 작성.
- [x] 탈출구 구현 — 다이얼로그 안에서 현재 세션 코드 발급·표시·복사,
...
- [x] 계약 pin: B2b(진입 응답 `has_save_code` — bootstrap과 redeem 양쪽,
      ...
      테스트로 작성.
- [x] `has_save_code` 서버 구현 — 세션 진입 wire shape 공유 지점에 가산.
- [x] 넛지 프론트 구현 — 판정은 순수 함수(누적 턴 수), 임계값·문구는
...
- [x] 다이얼로그 백드롭 클릭이 in-flight 중 `busy` 를 남기는 문제 수정 +
...
## Phase 7: Acceptance fixes (2차)

2차 수락 때 사람이 판정한 3건 — 백드롭 무피드백 완화, retro 문서 편입,
ESC 메뉴를 v1 목표로 확정하고 스펙에 반영.

Contracts:

- [x] 백드롭 무피드백 완화 — 요청 중 백드롭 클릭이 아무 반응 없이
      삼켜지는 것을 시각적으로 알린다. 순수 스타일 범위(커서/불투명도
      등 토큰 기반), 새 문구·새 요청·계약 변경 없음.
- [ ] `docs/retros/2026-08-19-talpi-0.5.0-dogfood.md` 를 레포에 편입 —
